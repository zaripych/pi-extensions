...
function tally(records: CostRecord[], keyOf: (r: CostRecord) => string) {
  const map = new Map<
    string,
    { total: number; tokens: number; calls: number; cwd: string }
  >()
  for (const r of records) {
    const key = keyOf(r)
  ...
  return [...map.entries()].toSorted((a, b) => b[1].total - a[1].total)
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand('cost', {
    description:
      'Show API cost summary. Usage: /cost [Nd] — e.g. /cost, /cost 7d, /cost 30d',
    handler: async (args, ctx) => {
      ...
      if (args && args.trim().length > 0) {
        ...
        if (parsed === null || parsed < 1) {
          ctx.ui.notify(
            'Usage: /cost [Nd] — e.g. /cost, /cost 7d, /cost 30d',
            'warning'
          )
          return
        }
      ...
      const { records, stats } = await collectSessionCostRecords({
        start,
        end: today,
      })
      ...
      const pushSection = (
        title: string,
        rows: { label: string; total: number; suffix: string }[]
      ) => {
      ...
      if (days === 1) {
        ...
        lines.push(
          theme.bold(
            theme.fg(
              'accent',
              `Cost for last ${days} days (${fmtDate(start)} → ${fmtDate(today)})`
            )
          )
        )
      }
      ...
      pushSection(
        'By model',
        tally(records, (r) => `${r.provider}/${r.model}`).map(
          ([model, data]) => ({
            label: model,
            total: data.total,
